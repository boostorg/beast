...
public:
    ...
    testOstream()
    {
        boost::core::string_view const s = "0123456789abcdef";
        ...
        // flush
        {
            // Issue #1853
            flat_static_buffer<16> b;
            auto half_view = boost::core::string_view(s.data(), 8);
            {
                auto os = ostream(b);
                os << half_view;
                os.flush();
...

        {
            ...
            {
                auto os = ostream(b);
                os << boost::core::string_view(s.data(), 8);
                os.flush();
                os << boost::core::string_view(s.data() + 8, 8);
                os.flush();
            }
            BEAST_EXPECT(buffers_to_string(b.data()) == s);
        }
