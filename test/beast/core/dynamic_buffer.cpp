...
public:
    ...
    void implTestV2Conversion(DynamicBuffer_v2 original)
    {
        ...
        BEAST_EXPECT(convertible_to_dynamic_buffer_v2<decayed_type>::value == true);
        BEAST_EXPECT(convertible_to_dynamic_buffer_v2<decayed_type&>::value == true);
        BEAST_EXPECT(convertible_to_dynamic_buffer_v2<decayed_type const&>::value == true);
        BEAST_EXPECT(convertible_to_dynamic_buffer_v2<decayed_type &&>::value == true);

        // taking dynamic_buffer of a DynamicBuffer_v2 results in a cheap
        // copy which refers to the same underlying data
        auto buffer = dynamic_buffer(original);
    ...
    testNetV2DynamicBuffers()
    {
        ...
        auto dd = dynamic_buffer(net_dyn_buffer);
